...
class PlotterSimulator:
	def __init__(self, step_time, phys_time, save_image=False, save_stats=True):
		...
		self.image_center_offset = (150,150)
	...
	def saveImage(self):
		while(self.current_phys_time > self.current_image_time and self.save_image):
			...
			file_name = "frames/%04d.png"%(self.image_count)
			print("Saving image "+str(file_name))
			
			img = PIL.Image.new("RGBA", self.image_size)
			...
			draw.line([effector_draw_x, effector_draw_y, effector_draw_x, effector_draw_y], fill=(255,0,0,255))
			draw.line([logical_draw_x, logical_draw_y, logical_draw_x, logical_draw_y], fill=(0,0,255,255))
			draw.line([effector_draw_x, effector_draw_y, logical_draw_x, logical_draw_y], fill=(0,255,0,255))
			draw.text((0,0),"%.5f"%(self.current_image_time), fill=(255,255,255,255))
			del draw
